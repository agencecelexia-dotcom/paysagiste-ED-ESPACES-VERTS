        ...
        <Container>
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mt-12 grid gap-8 md:grid-cols-2">
              <FadeIn delay={0.1}>
                <div className="rounded-xl bg-neutral-50 p-6">
                  <h2 className="text-xl font-bold text-neutral-900 mb-3">Le Défi</h2>
                  <p className="text-neutral-600 leading-relaxed">{project.challenge}</p>
                </div>
              </FadeIn>
              <FadeIn delay={0.2}>
                <div className="rounded-xl bg-primary-50 p-6">
                  <h2 className="text-xl font-bold text-neutral-900 mb-3">Notre Solution</h2>
            ...
            {project.beforeImage && project.afterImage && (
              <FadeIn className="mt-12">
                <h2 className="text-2xl font-bold text-neutral-900 mb-6">Avant / Après</h2>
            ...
            {project.images.length > 0 && (
              <FadeIn className="mt-12">
                <h2 className="text-2xl font-bold text-neutral-900 mb-6">Galerie Photos</h2>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
                  {project.images.map((image, index) => (
                    <div key={index} className="relative aspect-[4/3] overflow-hidden rounded-xl">
                      <Image src={image} alt={`${project.title} - Photo ${index + 1}`} fill className="object-cover" sizes="(max-width: 640px) 100vw, 50vw" />
                    </div>
                  ))}
                </div>
              </FadeIn>
            ...
            <FadeIn className="mt-16 text-center rounded-xl bg-primary-900 p-10">
              <h2 className="text-2xl font-bold text-white">Un projet similaire ?</h2>
              <p className="mt-2 text-neutral-200">Contactez-nous pour discuter de votre projet et obtenir un devis personnalisé.</p>
              <div className="mt-6">
          ...
          {relatedProjects.length > 0 && (
            <FadeIn className="mt-20">
              <h2 className="text-2xl font-bold text-neutral-900 mb-8 text-center">Projets Similaires</h2>
              <div className="grid grid-cols-1 gap-6 md:grid-cols-2 max-w-4xl mx-auto">
                {relatedProjects.map((rp) => (
                  <Link key={rp.id} href={`/realisations/${rp.slug}`} className="group relative block overflow-hidden rounded-xl aspect-[4/3]">
                    <Image src={rp.featuredImage} alt={rp.title} fill className="object-cover transition-transform duration-700 group-hover:scale-105" sizes="(max-width: 768px) 100vw, 50vw" />
